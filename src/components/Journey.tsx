          ...
          <div className="grid grid-cols-2 md:grid-cols-11 gap-4">
            {steps.map((step, index) => (
              <div key={index} className="text-center relative">
                {/* Dot */}
                <div className="w-4 h-4 rounded-full bg-[#c9a96e] mx-auto mb-3 relative z-10">
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 rounded-full border-2 border-[#c9a96e] opacity-50" />
                </div>
                {/* Label */}
                <p className="text-gray-400 text-xs leading-tight">{step}</p>
              </div>
            ))}
          </div>
        </div>
